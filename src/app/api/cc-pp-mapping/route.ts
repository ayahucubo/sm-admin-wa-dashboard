...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('CC PP Mapping GET request received');
    const { searchParams } = new URL(request.url);
    const action = searchParams.get('action');
    
    if (action === 'schema') {
      console.log('Fetching table schema...');
      const schema = await getCCPPMappingSchema();
    ...
    console.log('Fetching CC PP mapping data...');
    const data = await getCCPPMappingData();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const newRecord = await createCCPPMapping(body);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const updatedRecord = await updateCCPPMapping(id, updateData);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const deletedRecord = await deleteCCPPMapping(parseInt(id));
