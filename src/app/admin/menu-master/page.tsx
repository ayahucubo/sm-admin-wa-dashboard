          ...
          {editableColumns.map(column => {
            ...
            return (
              <div key={`field-${column}`}>
                ...
                {schemaCol?.data_type === 'text' || column.includes('description') || column.includes('keyword') ? (
                  <textarea
                    value={fieldValue}
...
const DeleteModal = memo(({ 
  ...
  isOpen: boolean;
  selectedRecord: MenuMaster | null;
  submitting: boolean;
  onClose: () => void;
  onDelete: () => void;
}) => {
...
export default function MenuMasterPage() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<MenuMaster | null>(null);
  ...
  const fetchSchema = async () => {
    try {
      ...
      if (result.success && result.schema && result.schema.length > 0) {
        ...
        // Fallback schema for Menu Master based on real table structure
      ...
      // Fallback schema for Menu Master
      const fallbackSchema = [
        { column_name: 'id', data_type: 'integer', is_nullable: 'NO', column_default: null },
        { column_name: 'company_code', data_type: 'character varying', is_nullable: 'YES', column_default: null },
        { column_name: 'menu', data_type: 'character varying', is_nullable: 'YES', column_default: null },
  ...
  const handleEditClick = useCallback((record: MenuMaster) => {
  ...
  const handleDeleteClick = useCallback((record: MenuMaster) => {
    setSelectedRecord(record);
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 sm:py-6 gap-4 sm:gap-0">
            <div className="flex items-center w-full sm:w-auto">
              ...
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-orange-600 rounded-lg flex items-center justify-center mr-3 sm:mr-4 flex-shrink-0">
                <svg className="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                </svg>
              </div>
              <div className="min-w-0 flex-1">
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">Menu Master Data Mapping</h1>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 line-clamp-2">
                  Database: n8n_param_menu_master ({filteredData.length} records)
                </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              ...
              <button
                onClick={fetchData}
                className="px-3 sm:px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors duration-200 flex items-center justify-center text-sm sm:text-base"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 sm:pl-12 pr-4 py-2 sm:py-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm sm:text-base"
                  value={searchTerm}
