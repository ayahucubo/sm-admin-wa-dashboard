...
import { exportToExcel, formatUniqueContactsForExcel, formatContactDetailsForExcel } from '@/utils/excelExport';
...
const UniqueContactsChart: React.FC = () => {
  ...
  // Handle Excel export for unique contacts data
  const handleExportUniqueContacts = () => {
    if (chartData.length === 0) {
      alert('No data available to export');
      return;
    }

    const excelData = formatUniqueContactsForExcel(chartData);
    const filename = `Unique_Contacts_Monitoring_${timePeriod}days_${new Date().toISOString().split('T')[0]}`;
    
    const success = exportToExcel(excelData, filename, 'Unique Contacts Data');
    if (success) {
      console.log('Unique contacts data exported to Excel successfully');
    } else {
      alert('Failed to export data to Excel');
    }
  };

  // Handle Excel export for contact details
  const handleExportContactDetails = () => {
    if (contactDetails.length === 0) {
      alert('No contact details available to export');
      return;
    }

    const excelData = formatContactDetailsForExcel(contactDetails, selectedDate);
    const filename = `Contact_Details_${new Date(selectedDate).toISOString().split('T')[0]}_${new Date().toISOString().split('T')[0]}`;
    
    const success = exportToExcel(excelData, filename, 'Contact Details');
    if (success) {
      console.log('Contact details exported to Excel successfully');
    } else {
      alert('Failed to export contact details to Excel');
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={handleExportUniqueContacts}
              disabled={loading || chartData.length === 0}
              className="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
            ...
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={handleExportContactDetails}
                    disabled={contactDetails.length === 0}
                    className="px-3 py-1 text-sm bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1"
                    title="Export contact details to Excel"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
