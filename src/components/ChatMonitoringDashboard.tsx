...
import { exportToExcel, formatChatVolumeForExcel } from '@/utils/excelExport';
...
export default function ChatMonitoringDashboard({ 
  ...
  const handleExportExcel = () => {
    ...
    const excelData = formatChatVolumeForExcel(chartData);
    const filename = `Chat_Volume_Monitoring_${currentPeriod.label.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}`;
    
    const success = exportToExcel(excelData, filename, 'Chat Volume Data');
    if (success) {
      console.log('Chat volume data exported to Excel successfully');
    } else {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 text-sm bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
